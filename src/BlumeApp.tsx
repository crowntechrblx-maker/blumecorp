...
export function BlumeApp({
  ...
  async function generatePersonReport() {
    ...
    try {
      ...
      doc.setFontSize(18);
      doc.setTextColor(20, 20, 20);
      doc.text("BLUME INTELLIGENCE REPORT", marginX, y);
      y += 22;
      ...
      doc.text(
        `Generated by ${username || "an unknown user"} on ${new Date().toLocaleString()}`,
        marginX,
        y
      );
      doc.setTextColor(20, 20, 20);
      ...
      if (personResult.groups.length === 0) {
        ...
        for (const g of personResult.groups) {
          line(`- ${g.name} (${g.tier === "red" ? "flagged" : "white"})`);
        }
      }
      spacer(6);
      if (personResult.groupScanChange) {
        ...
        line("No changes detected since the last group scan.");
      }
      spacer();
